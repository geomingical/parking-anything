...
test("validateMeta rejects inconsistent duration and provenance", () => {
  ...
  assert.deepEqual(validateAudioProvenance(validAudioProvenance, 3.1), validAudioProvenance);
  assert.throws(
    () => validateAudioProvenance({ ...validAudioProvenance, provider: "macOS say" }, 3.1),
    /audioProvenance\.provider/,
  );
  assert.throws(
    () => validateAudioProvenance({ ...validAudioProvenance, tempoFactor: 1 }, 3.1),
    /audioProvenance\.tempoFactor/,
  );
  assert.throws(
