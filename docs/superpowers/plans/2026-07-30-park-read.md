...
> The `ai_tool` label must render as `AI tool URL`, not `Tool URL` — an existing e2e test and several unit tests use that accessible name. Set `ai_tool`'s registry `label` usage here accordingly: use `kind === "ai_tool" ? "AI tool" : registry.label` for the field label only, and leave every other use on `registry.label`.
...
it("labels a read with its own kind wording and accent", () => {
  ...
  expect(screen.getByRole("button", { name: "On interface craft, Parked" })).toHaveStyle({
    "--kind-accent": "var(--kind-read)",
  });
});
```
