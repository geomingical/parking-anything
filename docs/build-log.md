...
### 2026-07-21 — v0.5 verified production release

- Received explicit production authorization, then promoted verified Preview `dpl_9SZMM62Tg267Nh7oKyt4fApGKqh2`. Vercel created ready production copy `dpl_Dm8Evvt8gEpgeEgwt34byn5fNCmB` at `https://parking-anything-mlc8gb9va-aim-ing-data.vercel.app` and assigned `https://parking-anything.vercel.app` to it. The prior v0.4 deployment `dpl_8R5o3DrmyPiE3vUyQ1ZbZinfFpHa` remains ready and recorded for rollback; no tag was pushed and the draft PR was not merged.
- Confirmed by metadata only that all 10 documented Production variable names remain configured as encrypted values: `OPENAI_API_KEY`, `OPENAI_ANALYZE_MODEL`, `OPENAI_PATROL_MODEL`, `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, `RATE_LIMIT_HASH_SECRET`, `DEMO_API_ENABLED`, `RATE_LIMIT_IP_MAX`, `RATE_LIMIT_IP_WINDOW_SECONDS`, and `RATE_LIMIT_GLOBAL_DAILY_MAX`. No value was retrieved or printed.
- The public alias opened without authentication and reset to the exact three v0.5 fixtures with no false storage warning. One real Tool analysis produced a schema-complete GPT-5.6 ticket and visible URL-only source warning. One real mixed Patrol reviewed Tool and Idea candidates together, kept Observed Fact separate from one GPT-5.6 Recommendation and two explicit deterministic fallbacks, and performed no automatic lifecycle mutation.
- Manual production UX verification completed Idea capture -> immediate same-record planning -> Test Drive -> evidence-backed shared Garage, plus reason-gated Tool Tow Away -> shared Scrapyard. The final `1 parked / 1 test-driving / 2 garaged / 1 scrapped` state survived reload.
- Isolated public-domain Playwright profiles passed exact strict-v1 migration and malformed-present-v2 authority on both desktop and mobile: four checks passed, v1 stayed byte-identical, converted v2 was authoritative, and malformed v2 plus valid v1 changed neither key.
- Mobile production verification at 390x844 reported `scrollWidth === clientWidth`; `Park whatever · Future` and `Gather · Future` remained visibly disabled. The browser console contained zero errors. A local `.next/static` scan found none of the server-only secret names or OpenAI-key-like patterns.
- Bounded logs for the first 30 production minutes showed the public page plus one `/api/analyze-url` and one `/api/manager-patrol` request; the error-only query returned no entries.
- Fresh release gate: `npm run lint` exited 0; `npm test` passed 23 files and 213 tests; `npm run test:e2e` passed 25 applicable tests with three documented skips; `npm run build` compiled `/`, `/api/analyze-url`, and `/api/manager-patrol`. A calendar-sensitive visual assertion initially expected exactly 49 days; the existing failing E2E test proved the issue, and the test-only correction now asserts the intended `Needs review · N days` contract without changing application behavior.
...
### 2026-07-21 — UI-first film redesign

- Rejected the prior typography-wall treatment because it made the film read as a deck before it read as a product demonstration. The accepted visual budget is approximately 70% real product capture, 20% purposeful vehicle/route motion, and 10% concise text.
- Rebuilt the 150-second review surface around nine product-led beats, the 149.2445-second / 362-word narration, 60 caption cues, and 14 deterministic snapshot times: 4, 23, 39, 53, 64, 80, 98, 110, 118, 128, 138, 142, 147, and 149.4 seconds.
- The browser contract verifies broken images and console/page errors; canvas and text overflow; frame-zero opacity; computed palette colors; minimum product widths; exact Evidence-or-Exit vehicle route checkpoints; a 45% visible `product-primary` union-area floor; Future disclosure at 110, 118, 128, and 138 seconds; and caption separation from product evidence, Future disclosure, and Gather.
- HyperFrames 0.7.65 `check` passes with zero runtime, layout, or motion errors and 22/22 WCAG AA text checks. Its four non-blocking lint warnings are three intentional first-party media-reuse notices and the existing 387-line maintainability notice. The 30-sample `inspect` returns zero errors and zero warnings; intentional screenshot crops and transition coverage remain info-level only.
- Visual inspection accepted the capture-led hierarchy, single-headline/two-label ceiling, distinct adjacent layouts, purpose-driven cars, honest Future roadmap, and subordinate captions. It rejected one demonstrated 98-second line-break defect, then kept `GPT-5.6` together and regenerated the complete contact sheet.
- Protected teaser and narration/transcript/caption assets remain byte-identical to their recorded baselines. No MP4 was rendered, and no product runtime, deployment, push, PR, secret, or production state was changed; export remains gated on explicit human Studio approval.

### 2026-07-21 — UI-first video QA artifact correction

- Withdrew the initial visual-artifact acceptance after an order-dependent random-access capture exposed an empty or clipped Future disclosure. The DOM remained correct; the renderer could retain a transition cover outside its active window because cover transform/visibility had no complete timeline state.
- Made transition covers hidden by default, explicitly reset and reveal each at its transition start, hide it again after 0.82 seconds, and retain a timeline-zero transform state. The local contract now verifies exact disclosure text, 363.5×54px box geometry, 317.5×23px text geometry, clipping, full scene opacity, and a visible Future layer through 138.4 seconds.
- Replaced the capture helper with one canonical main-only generator: 14 stable indexed 1920×1080 PNGs, one singular 1920×1080 4×4 contact sheet built with local `sharp`, safe cleanup limited to known main artifacts, and no teaser writes. It rejects a collapsed yellow box, fewer than 900 dark text pixels or a 290px text span, and low-deviation blank Future scenes.
- Two consecutive final generator runs produced byte-identical 15-file manifests. At 110, 118, 128, and 138 seconds each disclosure measured 15,085 yellow pixels over 357×48 and 1,297 dark text pixels over 312×15; scene channel deviation remained 38.67–94.46. A fresh-path direct inspection confirmed the full 138-second Future scene and disclosure text.
- The pinned HyperFrames renderer remains a verification gate in a temporary output directory; it no longer owns or renames the committed canonical evidence. Protected teaser and narration/transcript/caption artifacts remain outside the generator's write scope. MP4 export remains gated on explicit human Studio approval.
- HyperFrames 0.7.65 `check` passed runtime, layout, motion, and 22/22 contrast checks with exactly four non-blocking lint warnings: three intentional first-party duplicate-media notices and the 390-line composition notice. The deprecated 30-sample `inspect` completed with zero errors, zero warnings, and 28 crop/transition info items. The full repository gate passed ESLint, 25 Vitest files / 216 tests, TypeScript, and the Next.js production build; the first sandboxed build failed only because Turbopack could not bind an internal port and passed unchanged with that restriction lifted.
- Added a standalone Node integration contract outside the Vitest glob. It starts and stops its own loopback server, directs the real generator to a temporary directory, checks the exact 14-frame-plus-contact-sheet inventory and 1920×1080 dimensions, compares byte-identical manifests across two runs, hashes the full teaser tree before and after, and removes only its owned temporary directory.
